...
namespace PowerBIEmbedded_AppOwnsData.Services
{
    public class EmbedService : IEmbedService
    {
        ...
        private static readonly string SPObjectId = sectionConfig["spobjectid"];
        private static readonly string SProle = sectionConfig["sprole"];
            ...
            try
            {
                ...
                using (var client = new PowerBIClient(new Uri(ApiUrl), m_tokenCredentials))
                {
                    ...
                    else
                    {
                        ...
                        //Line 121 & Line 122 is for ServicePrincipal
                        var rls = new EffectiveIdentity(SPObjectId, new List<string> { report.DatasetId }, new[] { SProle });
                        generateTokenRequestParameters = new GenerateTokenRequest(accessLevel: "view", identities: new List<EffectiveIdentity> { rls });
                        //Line 124 is for MasterUser
